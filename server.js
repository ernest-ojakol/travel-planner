...
app.get('/favorites', async (req, res) => {
  try {
    ...
    const favoritesData = await fs.readFile(path.join(__dirname, 'favorites.json'), 'utf8');
    const favorites = JSON.parse(favoritesData);
    res.render('favorites', { favorites, googleMapsApiKey: process.env.GOOGLE_MAPS_API_KEY });
  } catch (error) {
    console.error(error);
    res.render('favorites', { favorites: [], error: 'Failed to load favorites.', googleMapsApiKey: process.env.GOOGLE_MAPS_API_KEY });
  }
});
